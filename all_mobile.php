...
@media screen and (max-width: 768px) {
    ...
    /* Filtres checkbox - mieux organisés */
    .checkbox-section {
        width: 100%;
    }
...
function initMobileAccordion() {
    ...
    rows.forEach(row => {
        ...
        // Cotisation
        const cotiCell = cells[cells.length - 5];
        const cotiInput = cotiCell.querySelector('input');
        const cotiValue = cotiInput ? cotiInput.value : '';
        const cotiSelected = cotiCell.classList.contains('selected') ? 'selected' : '';
        detailsHTML += `
        ...
        // Facturé
        const factureCell = cells[cells.length - 4];
        const factureClasses = factureCell.className;
        ...
        detailsHTML += `
            <div class="action-item ${factureBlue}">
                ...
                ${factureCell.innerHTML}
            </div>
        `;
        
        // Repris
        const reprisCell = cells[cells.length - 3];
        const reprisClasses = reprisCell.className;
        ...
        detailsHTML += `
            <div class="action-item ${repriseOrange}">
                ...
                ${reprisCell.innerHTML}
            </div>
        `;
        
        // Boutons Terminé, Modifier, Supprimer
        const termineCell = cells[cells.length - 2];
        const editCell = cells[cells.length - 1];
        const deleteCell = cells[cells.length];
